...
22、上接No 18.因为.gitignore文件的存在,vendor以及.env文件不会被同步到github代码库中,墙裂不建议把这两项从.gitignore中剔除,因为laravel发展到现在官方自己就收录了很多第三方的轮子,导致一个vendor目录动辄几十甚至上百兆,关键是碎片文件太多,不忽略的确不是个明智之举,那新人进组之后应该怎么做呢,总不能线下找老员工把缺失的文件传过来吧.正确的做法如下:
...
解决这个bug的过程真正的感受到了'山重水复疑无路,船到桥头自然沉',最后快要放弃的时候在故纸堆里一块好久没人维护的代码里找到了类似的解决方案,才知道js还有Blob这么个对象.过后自己有道网上查关于Blob的资料才发现有很多前人已经给出了很完美的解决方案,只是我没找到而已,关键是不知道Blob对象,所以关键字完全对不上,不管怎么突破盲点的感觉很爽...
